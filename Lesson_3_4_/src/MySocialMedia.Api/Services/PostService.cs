...
public class PostService : IPostService
{
    ...
    public Result<Guid> Add(Token token, PostAddDto dto)
    {
        var access = EnsureAccess(token);
        if (!access.Success) return Result<Guid>.Fail(access.Error!);

        if (dto is null) return Result<Guid>.Fail("Ma'lumot yo'q.");
        if (string.IsNullOrWhiteSpace(dto.Title)) return Result<Guid>.Fail("Title bo'sh bo'lishi mumkin emas");
        if (string.IsNullOrWhiteSpace(dto.Content)) return Result<Guid>.Fail("Content bo'sh bo'lishi mumkin emas");
